...
/*
	...
	true if only image uploads (PNG, WEBP, GIF, JPEG) are allowed. To apply a maximum width or height (in pixels) to uploaded
...
*/
	...
	//	Find out what type of source file was uploaded and if appropriate, check it's an image and get preliminary size measure
		...
		$isimage=($format=='png') || ($format=='gif') || ($format=='webp') || ($format=='jpeg') || ($format=='jpg'); // allowed image extensions
		...
		if ($onlyimage)
			if ( (!$isimage) || !is_array($imagesize) ) {
				$result['error']=qa_lang_sub('main/image_not_read', 'GIF, JPG, PNG, WEBP');
				return $result;
			}
