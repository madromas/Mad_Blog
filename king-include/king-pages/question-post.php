...
*/
	{
		...
		$form=array(
			'tags' => 'method="post" enctype="multipart/form-data" action="'.qa_self_html().'"',
				...
				'thumb' => array(
					...
					'html' => '<div class="king-form-group">
								<span class="inputprev-span">
									<img class="inputprev" src="'.qa_html(isset($in['content']) ? $in['content'] : $question['content']).'" /><i class="fas fa-cloud-upload-alt fa-2x"></i>
	...
	if ($question['postformat'] == 'I') {
		$imagesedit = preg_replace('(https*:\/\/[-\[\]\{\}\(\)\%_\/.a-zA-Z0-9+]+\.(png|jpg|webp|jpeg|gif|bmp))', '<div class="submit-images"><img class="imagesedit" src="$0"/><input id="download" class="hide" value="$0" name="submit_image[]"><span class="king-remove"><i class="fas fa-trash"></i></span></div>', $edit);
	    $form['fields']['tiny']['html'].='<div id="multipleupload">Upload</div>'.$imagesedit.'';
	    $form['fields']['tiny']['label'].= qa_lang_html('main/nav_type_image');
	} elseif ($question['postformat'] == 'V') {
		$form['fields']['tiny']['html'].='<input type="text" name="q_extra" class="king-form-tall-text" value="'.$edit.'">';
		$form['fields']['tiny']['label'].= qa_lang_html('main/nav_type_video');
	} else {
		$form['fields']['tiny']['html'].='<div id="q_extra">'.$edit.'</div>';
		$form['fields']['tiny']['label'].= qa_lang_html('main/news_content');
	}		
...
*/
	{
		...
		if (!qa_check_form_security_code('edit-'.$question['postid'], qa_post_text('code')))
			...
			if ( !empty($_FILES['king_post_files']['tmp_name']) || is_uploaded_file($_FILES['king_post_files']['tmp_name'])) {
				...
				$in['content'] = king_uploadthumb($ImageName,$TempSrc,$ImageType);
			}
			if ($question['postformat'] == 'I') {
			    $values = array();
			    $members = isset($_POST['submit_image']) ? $_POST['submit_image'] : null;
			    for($i=0 ;$i < count($members); $i++) {
			        $values[] = $members[$i];
			    }

				$in['extra']=implode(' ', $values);
			}
...
*/
	{
		...
		elseif ($in['duplicate']) {
			...
			// a) URLs could have a fixed numerical path, e.g. https://qa.mysite.com/1/478/...
			// b) There could be a question title which is just a number, e.g. https://qa.mysite.com/478/12345/...
